...
// Convert a token back to bytes using the BPE map
// Returns number of bytes written to output buffer
size_t token_to_bytes2(uint32_t token, char* output, size_t max_len, Map map) {
    // Base case: if token is a byte, write it directly
    if (token < 256) {
        if (max_len < 1) return 0;  // Check buffer space
        output[0] = (char)token;
        return 1;
    }
    ...
    for (size_t i = 0; i < map.count; i++) {
        if (map.items[i].value == token) {
            // Recursively decode left component
            size_t left_bytes = token_to_bytes2(map.items[i].l, output, max_len, map);
            if (left_bytes == 0) return 0;  // Error or buffer full
            bytes_written += left_bytes;

            // Recursively decode right component
            if (bytes_written >= max_len) return bytes_written;  // Buffer full
            size_t right_bytes = token_to_bytes2(map.items[i].r, 
                                               output + left_bytes, 
                                               max_len - left_bytes, 
                                               map);
...
size_t token_to_bytes(uint32_t token, char* output, size_t max_len, Map map) {
    ...
    if (!kv) {
        fprintf(stderr, "Error: Token %d not found in map\n", token);
...
int main() { 
    ...
    size_t bytes_written = token_to_bytes2((uint32_t)max_idx, output_buffer, max_len, map);
    ...
    print_string_from_token((uint32_t)max_idx, map);
    printf("\n");
